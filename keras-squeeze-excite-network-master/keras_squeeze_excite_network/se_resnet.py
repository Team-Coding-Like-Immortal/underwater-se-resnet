...
if TF:
    ...
    from tensorflow.keras.layers import (Activation, BatchNormalization, Conv2D, Dense,
                                         ...
                                         Input, MaxPooling2D, add, DepthwiseConv2D, SeparableConv2D)
    ...
    from keras.layers import (Activation, BatchNormalization, Conv2D, Dense,
                              GlobalAveragePooling2D, GlobalMaxPooling2D,
                              Input, MaxPooling2D, add, DepthwiseConv2D)
...
def _resnet_block(input_tensor, filters, k=1, strides=(1, 1)):
    ...
    if strides != (1, 1) or _tensor_shape(init)[channel_axis] != filters * k:
        ...
        # init = SeparableConv2D(filters * k, (1, 1), padding='same', kernel_initializer='he_normal',
        #               use_bias=False, strides=strides)(x)

        # init = DepthwiseConv2D(filters * k, (1, 1), padding='same', kernel_initializer='he_normal',
        #                        use_bias=False, strides=strides)(x)
